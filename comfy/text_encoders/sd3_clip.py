...
class T5XXLModel(sd1_clip.SDClipModel):
    def __init__(self, device="cpu", layer="last", layer_idx=None, dtype=None, attention_mask=False, model_options={}):
        ...
        super().__init__(device=device, layer=layer, layer_idx=layer_idx, textmodel_json_config=textmodel_json_config, dtype=dtype, special_tokens={"end": 1, "pad": 0}, model_class=vgs.submodules.comfyui.comfy.text_encoders.t5.T5, enable_attention_masks=attention_mask, return_attention_masks=attention_mask, model_options=model_options)
...
class SD3ClipModel(torch.nn.Module):
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if out is None:
            out = torch.zeros((1, 77, 4096), device=vgs.submodules.comfyui.comfy.model_management.intermediate_device())
