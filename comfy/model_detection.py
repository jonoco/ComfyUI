import json
import vgs.submodules.comfyui.comfy.supported_models
import vgs.submodules.comfyui.comfy.supported_models_base
...
def model_config_from_unet_config(unet_config, state_dict=None):
    for model_config in vgs.submodules.comfyui.comfy.supported_models.models:
...
def model_config_from_unet(state_dict, unet_key_prefix, use_base_if_no_match=False, metadata=None):
    ...
    if model_config is None and use_base_if_no_match:
        model_config = vgs.submodules.comfyui.comfy.supported_models_base.BASE(unet_config)
...
def convert_diffusers_mmdit(state_dict, output_prefix=""):
    ...
    if 'joint_transformer_blocks.0.attn.add_k_proj.weight' in state_dict: #AuraFlow
        ...
        sd_map = vgs.submodules.comfyui.comfy.utils.auraflow_to_diffusers({"n_double_layers": num_joint, "n_layers": num_joint + num_single}, output_prefix=output_prefix)
        ...
        sd_map = vgs.submodules.comfyui.comfy.utils.pixart_to_diffusers({"depth": num_blocks}, output_prefix=output_prefix)
    elif 'x_embedder.weight' in state_dict: #Flux
        depth = count_blocks(state_dict, 'transformer_blocks.{}.')
        depth_single_blocks = count_blocks(state_dict, 'single_transformer_blocks.{}.')
        hidden_size = state_dict["x_embedder.bias"].shape[0]
        sd_map = vgs.submodules.comfyui.comfy.utils.flux_to_diffusers({"depth": depth, "depth_single_blocks": depth_single_blocks, "hidden_size": hidden_size}, output_prefix=output_prefix)
    elif 'transformer_blocks.0.attn.add_q_proj.weight' in state_dict and 'pos_embed.proj.weight' in state_dict: #SD3
        num_blocks = count_blocks(state_dict, 'transformer_blocks.{}.')
        depth = state_dict["pos_embed.proj.weight"].shape[0] // 64
        sd_map = vgs.submodules.comfyui.comfy.utils.mmdit_to_diffusers({"depth": depth, "num_blocks": num_blocks}, output_prefix=output_prefix)
    else:
        return None
