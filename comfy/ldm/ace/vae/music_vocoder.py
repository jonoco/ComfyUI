...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        if self.data_format == "channels_last":
            return F.layer_norm(
                x, self.normalized_shape, vgs.submodules.comfyui.comfy.model_management.cast_to(self.weight, dtype=x.dtype, device=x.device), vgs.submodules.comfyui.comfy.model_management.cast_to(self.bias, dtype=x.dtype, device=x.device), self.eps
            )
        elif self.data_format == "channels_first":
            u = x.mean(1, keepdim=True)
            s = (x - u).pow(2).mean(1, keepdim=True)
            x = (x - u) / torch.sqrt(s + self.eps)
            x = vgs.submodules.comfyui.comfy.model_management.cast_to(self.weight[:, None], dtype=x.dtype, device=x.device) * x + vgs.submodules.comfyui.comfy.model_management.cast_to(self.bias[:, None], dtype=x.dtype, device=x.device)
            return x
...
class ConvNeXtBlock(nn.Module):
    ...
    def forward(self, x, apply_residual: bool = True):
        ...
        if self.gamma is not None:
            x = vgs.submodules.comfyui.comfy.model_management.cast_to(self.gamma, dtype=x.dtype, device=x.device) * x
