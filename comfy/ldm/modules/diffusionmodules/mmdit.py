...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        if self.dynamic_img_pad:
            x = vgs.submodules.comfyui.comfy.ldm.common_dit.pad_to_patch_size(x, self.patch_size, padding_mode=self.padding_mode)
        x = self.proj(x)
        if self.flatten:
...
class RMSNorm(torch.nn.Module):
    ...
    def forward(self, x):
        return vgs.submodules.comfyui.comfy.ldm.common_dit.rms_norm(x, self.weight, self.eps)
...
class MMDiT(nn.Module):
    ...
    def forward(
        ...
        hw = x.shape[-2:]
        x = self.x_embedder(x) + vgs.submodules.comfyui.comfy.ops.cast_to_input(self.cropped_pos_embed(hw, device=x.device), x)
