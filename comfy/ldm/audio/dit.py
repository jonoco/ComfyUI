...
class FourierFeatures(nn.Module):
    ...
    def forward(self, input):
        f = 2 * math.pi * input @ vgs.submodules.comfyui.comfy.ops.cast_to_input(self.weight.T, input)
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        beta = self.beta
        if beta is not None:
            beta = vgs.submodules.comfyui.comfy.ops.cast_to_input(beta, x)
        return F.layer_norm(x, x.shape[-1:], weight=vgs.submodules.comfyui.comfy.ops.cast_to_input(self.gamma, x), bias=beta)
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, t):
        ...
        freqs = torch.einsum('i , j -> i j', t, vgs.submodules.comfyui.comfy.ops.cast_to_input(self.inv_freq, t))
        ...
        scale = vgs.submodules.comfyui.comfy.ops.cast_to_input(self.scale, t) ** rearrange(power, 'n -> n 1')
