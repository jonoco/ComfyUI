...
from vgs.submodules.comfyui.comfy.ldm.lightricks.model import Timesteps
...
class OmniGen2Transformer2DModel(nn.Module):
    ...
    def flat_and_pad_to_seq(self, hidden_states, ref_image_hidden_states):
        ...
        if ref_image_hidden_states is not None:
            ref_image_hidden_states = list(map(lambda ref: vgs.submodules.comfyui.comfy.ldm.common_dit.pad_to_patch_size(ref, (p, p)), ref_image_hidden_states))
    ...
    def img_patch_embed_and_refine(self, hidden_states, ref_image_hidden_states, padded_img_mask, padded_ref_img_mask, noise_rotary_emb, ref_img_rotary_emb, l_effective_ref_img_len, l_effective_img_len, temb):
        ...
        if ref_image_hidden_states is not None:
            ...
            image_index_embedding = vgs.submodules.comfyui.comfy.model_management.cast_to(self.image_index_embedding, dtype=hidden_states.dtype, device=hidden_states.device)
    ...
    def forward(self, x, timesteps, context, num_tokens, ref_latents=None, attention_mask=None, **kwargs):
        B, C, H, W = x.shape
        hidden_states = vgs.submodules.comfyui.comfy.ldm.common_dit.pad_to_patch_size(x, (self.patch_size, self.patch_size))
