...
def modulated_rmsnorm(x, scale, eps=1e-6):
    ...
    x_normed = vgs.submodules.comfyui.comfy.ldm.common_dit.rms_norm(x, eps=eps)
...
def residual_tanh_gated_rmsnorm(x, x_res, gate, eps=1e-6):
    ...
    x_normed = vgs.submodules.comfyui.comfy.ldm.common_dit.rms_norm(x_res, eps=eps) * tanh_gate
...
class AsymmDiTJoint(nn.Module):
    ...
    def prepare(
        ...
        rope_cos, rope_sin = compute_mixed_rotation(
            freqs=vgs.submodules.comfyui.comfy.ops.cast_to(self.pos_frequencies, dtype=x.dtype, device=x.device), pos=pos
