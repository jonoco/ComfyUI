...
def calculate_weight(patches, weight, key, intermediate_dtype=torch.float32, original_weights=None):
    for p in patches:
        ...
        if isinstance(v, list):
            v = (calculate_weight(v[1:], v[0][1](vgs.submodules.comfyui.comfy.model_management.cast_to_device(v[0][0], weight.device, intermediate_dtype, copy=True), inplace=True), key, intermediate_dtype=intermediate_dtype), )
