import torch
from vgs.submodules.comfyui.comfy.text_encoders.bert import BertAttention
...
class LayerScale(torch.nn.Module):
    ...
    def forward(self, x):
        return x * vgs.submodules.comfyui.comfy.model_management.cast_to_device(self.lambda1, x.device, x.dtype)
...
class Dino2Embeddings(torch.nn.Module):
    ...
    def forward(self, pixel_values):
        ...
        x = x + vgs.submodules.comfyui.comfy.model_management.cast_to_device(self.position_embeddings, x.device, x.dtype)
        return x
