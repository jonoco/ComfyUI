...
class MLP(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_dim, use_residual=True, operations=vgs.submodules.comfyui.comfy.ops):
        super().__init__()
        if use_residual:
...
class PhotoMakerIDEncoder(vgs.submodules.comfyui.comfy.clip_model.CLIPVisionModelProjection):
    def __init__(self):
        ...
        dtype = vgs.submodules.comfyui.comfy.model_management.text_encoder_dtype(self.load_device)

        super().__init__(VISION_CONFIG_DICT, dtype, offload_device, vgs.submodules.comfyui.comfy.ops.manual_cast)
        self.visual_projection_2 = vgs.submodules.comfyui.comfy.ops.manual_cast.Linear(1024, 1280, bias=False)
        self.fuse_module = FuseModule(2048, vgs.submodules.comfyui.comfy.ops.manual_cast)
...
class PhotoMakerLoader:
    ...
    def load_photomaker_model(self, photomaker_model_name):
        ...
        data = vgs.submodules.comfyui.comfy.utils.load_torch_file(photomaker_model_path, safe_load=True)
...
class PhotoMakerEncode:
    ...
    def apply_photomaker(self, photomaker, image, clip, text):
        ...
        pixel_values = vgs.submodules.comfyui.comfy.clip_vision.clip_preprocess(image.to(photomaker.load_device)).float()
        try:
