...
def sample_lcm_upscale(model, x, sigmas, extra_args=None, callback=None, disable=None, total_upscale=2.0, upscale_method="bislerp", upscale_steps=None):
    ...
    for i in trange(len(sigmas) - 1, disable=disable):
        ...
        x = denoised
        if i < len(upscales):
            x = vgs.submodules.comfyui.comfy.utils.common_upscale(x, round(orig_shape[-1] * upscales[i]), round(orig_shape[-2] * upscales[i]), upscale_method, "disabled")
...
class SamplerLCMUpscale:
    ...
    def get_sampler(self, scale_ratio, scale_steps, upscale_method):
        if scale_steps < 0:
            scale_steps = None
        sampler = vgs.submodules.comfyui.comfy.samplers.KSAMPLER(sample_lcm_upscale, extra_options={"total_upscale": scale_ratio, "upscale_steps": scale_steps, "upscale_method": upscale_method})
        return (sampler, )

from vgs.submodules.comfyui.comfy.k_diffusion.sampling import to_d
...
class SamplerEulerCFGpp:
    ...
    def get_sampler(self, version):
        if version == "alternative":
            sampler = vgs.submodules.comfyui.comfy.samplers.KSAMPLER(sample_euler_pp)
        else:
            sampler = vgs.submodules.comfyui.comfy.samplers.ksampler("euler_cfg_pp")
        return (sampler, )
